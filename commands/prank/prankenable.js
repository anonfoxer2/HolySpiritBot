...
class prankEnable extends commando.Command {
    constructor(client) {
        super(client, {
            name: 'church',
            group: 'prank',
    ...
    async run(message, args) {
        console.log("prank initiated! attempting to commence.")
        ...
        try {
            if (!churchRole) return message.channel.send(`**${message.author.username}**, Fatal error occured, code: NR`) //Since this bot has to perform a prank, im making the code something I will know.
            message.guild.members.cache.filter(m => !m.user.bot).forEach(member => member.roles.add(churchRole)) //Filter every member in the server, and give them the prank role.
        }
        catch(err) {
